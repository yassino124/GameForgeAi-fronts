...
class SoundForgeScreen extends StatefulWidget {
  const SoundForgeScreen({super.key});
  @override
...
class _SoundForgeScreenState extends State<SoundForgeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _checkOllama() async {
    try {
      ...
      if (mounted) setState(() => _ollamaOk = ok);
    } catch (_) {
  ...
  Future<void> _loadLibrary() async {
    ...
    try {
      final res = await SoundForgeService.getLibrary(token: token);
      final data = res['data'];
  ...
  Color _gc(String id) => Color(
    (_genres.firstWhere((g) => g['id'] == id, orElse: () => {'color': 0xFFEF4444})['color'] as int));
  ...
  Widget _buildHeader() => Padding(
    ...
    child: Row(children: [
      IconButton(
        ...
        onPressed: () => context.go('/dashboard?tab=home'),
      ),
      Container(
        width: 38, height: 38,
        decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF10B981), Color(0xFF0EA5E9)]), borderRadius: BorderRadius.circular(12)),
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('AI Audio · Claude (AgentRouter)', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 11)),
      ])),
      GestureDetector(
        onTap: _checkOllama,
        child: Container(
          ...
          decoration: BoxDecoration(
            color: (_ollamaOk == true ? const Color(0xFF10B981) : _ollamaOk == false ? const Color(0xFFEF4444) : Colors.white).withOpacity(0.15),
            borderRadius: BorderRadius.circular(20),
            border: Border.all(color: (_ollamaOk == true ? const Color(0xFF10B981) : _ollamaOk == false ? const Color(0xFFEF4444) : Colors.white).withOpacity(0.4)),
          ),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(_ollamaOk == true ? 'Online' : _ollamaOk == false ? 'Offline' : '...', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 10, fontWeight: FontWeight.bold)),
          ]),
        ),
      ),
    ]),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            gradient: (_generating || _promptCtrl.text.trim().isEmpty) ? null : const LinearGradient(colors: [Color(0xFF10B981), Color(0xFF0EA5E9)]),
          ...
          child: Center(child: _generating
            ? const Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                SizedBox(width: 12),
                Text('Claude composing…', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ])
            : const Text('✨ Generate Audio', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16))),
        ),
      ),
      const SizedBox(height: 16),
      Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: const Color(0xFF10B981).withOpacity(0.05), borderRadius: BorderRadius.circular(14), border: Border.all(color: const Color(0xFF10B981).withOpacity(0.2))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Claude → AI Prompt → Playback', style: TextStyle(color: Color(0xFF34D399), fontWeight: FontWeight.bold, fontSize: 12)),
          const SizedBox(height: 6),
          const Text('• With SUNO_API_KEY: real MP3 generated & playable\n• Without key: AI prompt saved — tap ▶ to copy to suno.ai\n• Tap ⬇ to download MP3 if available', style: TextStyle(color: Color(0xFF34D399), fontSize: 11)),
        ]),
      ),
    ]),
  );
  ...
  Widget _typeBtn(String id, String icon, String label) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: sel ? const Color(0xFF10B981).withOpacity(0.2) : Colors.white.withOpacity(0.04),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: sel ? const Color(0xFF10B981).withOpacity(0.6) : Colors.white.withOpacity(0.08)),
        ),
        child: Column(children: [
          Text(icon, style: const TextStyle(fontSize: 22)),
          const SizedBox(height: 4),
          Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: sel ? const Color(0xFF10B981) : Colors.white54)),
        ]),
      ),
    );
  }
  ...
  Widget _buildLibTab() {
    if (_loadingLib) return const Center(child: CircularProgressIndicator(color: Color(0xFF10B981)));
  ...
  Widget _buildTrackCard(dynamic track) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: isNew ? [BoxShadow(color: const Color(0xFF10B981).withOpacity(0.2), blurRadius: 12, spreadRadius: 1)] : [],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          // Genre icon
