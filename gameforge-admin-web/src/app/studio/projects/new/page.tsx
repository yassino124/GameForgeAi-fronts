...
function TemplatePurchasePaymentForm({
  amountUsd,
  onCancel,
  onBack,
  onSuccess,
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
        <div className="text-[10px] uppercase tracking-[0.28em] text-zinc-500 font-black">Total payment</div>
        <div className="mt-1 text-xl font-black text-white">${Number(amountUsd || 0).toFixed(2)}</div>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex-1 rounded-xl border border-cyan-500/35 bg-gradient-to-r from-blue-700 to-sky-500 px-4 py-3 text-sm font-black uppercase tracking-wide text-white disabled:opacity-40"
        >
...
function CreateProjectWizardInner() {
  ...
  const [purchasePaymentIntentId, setPurchasePaymentIntentId] = useState<string | null>(null);
  ...
  const selectedTemplateId = String(selectedTemplate?.id || (selectedTemplate as any)?._id || "").trim();
  const selectedTemplatePrice = Math.max(0, Number(selectedTemplate?.price || 0));
  ...
  const closePurchaseModal = () => {
    ...
    setPurchasePaymentIntentId(null);
  };

  const hasTemplateAccess = async (templateId: string) => {
    const res = await apiFetch<any>(`/templates/${encodeURIComponent(templateId)}/access`, { method: "GET", token });
    return Boolean(res?.hasAccess);
  };

  const initializeTemplatePurchase = async (templateId: string) => {
    if (!token) return;
    if (!STRIPE_PUBLISHABLE_KEY || !stripePromise) {
      setPurchaseError("Stripe key missing on frontend (NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY).");
      return;
    }
    ...
    try {
      const res = await apiFetch<any>(`/templates/${encodeURIComponent(templateId)}/purchase/payment-sheet`, {
        method: "POST",
        token,
      });
      ...
      if (!clientSecret) throw new Error("Missing Stripe client secret from payment-sheet response.");

      setPurchaseClientSecret(clientSecret);
      setPurchasePaymentIntentId(paymentIntentId || null);
    } catch (e: any) {
      setPurchaseError(e?.message || "Could not initialize secure payment.");
    } finally {
      setPurchaseBusy(false);
    }
  };

  const confirmTemplatePurchase = async (templateId: string, paymentIntentId?: string) => {
    const finalPaymentIntentId = String(paymentIntentId || purchasePaymentIntentId || "").trim();
    if (!finalPaymentIntentId) throw new Error("Missing payment intent id for confirmation.");

    await apiFetch<any>(`/templates/${encodeURIComponent(templateId)}/purchase/confirm`, {
      method: "POST",
      token,
      body: { paymentIntentId: finalPaymentIntentId },
    });
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      if (selectedTemplatePrice > 0 && selectedTemplateId) {
        const access = await hasTemplateAccess(selectedTemplateId);
        if (!access) {
          await initializeTemplatePurchase(selectedTemplateId);
          return;
        }
      }
      await executeProjectCreation();
    } catch (e: any) {
      setError(e instanceof ApiError ? e.message : (e?.message || "Creation failed"));
    }
  };
      ...
      <div className="max-w-7xl mx-auto pb-20">
        ...
        <AnimatePresence>
          {loading && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[var(--gf-panel-bg)]/90 backdrop-blur-xl"
            >
              <div className="relative h-96 w-96 flex items-center justify-center">
                ...
                <motion.div
                  ...
                  className="h-40 w-40 rounded-[48px] bg-gradient-to-br from-blue-700 to-sky-500 flex flex-col items-center justify-center border border-white/20 shadow-2xl relative z-10"
                >
                  ...
                  <div className="flex gap-2 justify-center">
                    {Array.from({ length: 10 }).map((_, i) => (
                      <motion.div
                        ...
                        className="h-1.5 w-6 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                      />
                    ))}
                  </div>
                </div>
          ...
          {purchaseModalOpen ? (
            <div className="fixed inset-0 z-[140] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative w-full max-w-xl gf-panel-strong rounded-[28px] border border-white/10 p-6 shadow-2xl"
              >
                <button
                  type="button"
                  onClick={closePurchaseModal}
                  className="absolute right-4 top-4 rounded-xl border border-white/10 bg-white/5 p-2 text-zinc-400 hover:text-white"
                >
                  <X size={16} />
                </button>

                <div className="text-lg font-black text-white">Unlock paid template</div>
                <div className="mt-1 text-sm text-zinc-400">
                  You need to complete secure card payment before starting this project from <span className="text-white">{selectedTemplate?.name}</span>.
                </div>
                ...
                <div className="mt-5">
                  {purchaseBusy ? (
                    ...
                    <div className="space-y-4 rounded-2xl border border-rose-500/35 bg-rose-500/10 p-4">
                      <div className="text-sm font-semibold text-rose-200">Payment initialization failed</div>
                      <div className="text-xs text-rose-100/90">{purchaseError}</div>
                      <button
                        className="rounded-xl border border-white/15 bg-white/[0.03] px-4 py-2 text-xs font-black uppercase tracking-wide text-zinc-200"
                        onClick={() => selectedTemplateId && initializeTemplatePurchase(selectedTemplateId)}
                      >
                        Retry
                      </button>
                    </div>
                  ) : purchaseClientSecret && stripePromise ? (
                    <Elements
                    ...
                    >
                      <TemplatePurchasePaymentForm
                        amountUsd={selectedTemplatePrice}
                        onBack={closePurchaseModal}
                        onCancel={closePurchaseModal}
                        onSuccess={async (paymentIntentId) => {
                          try {
                            setPurchaseBusy(true);
                            await confirmTemplatePurchase(selectedTemplateId, paymentIntentId);
                            closePurchaseModal();
                            await executeProjectCreation();
                          } catch (e: any) {
                            setPurchaseError(e?.message || "Could not confirm template purchase.");
                          } finally {
                            setPurchaseBusy(false);
                          }
                        }}
            ...
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="mt-5 flex gap-2">
                  ...
                  <button
                    className="flex-1 rounded-xl bg-blue-500 px-4 py-2 text-sm font-black text-white"
        ...
        <div className="mb-12 flex items-center justify-center">
          <div className="flex items-center gap-4 bg-white/[0.03] border border-white/5 p-2 rounded-[24px] backdrop-blur-xl shadow-2xl">
            {[
              ...
              { id: 2, label: "DNA Config", icon: Cpu },
              { id: 3, label: "Engine Launch", icon: Rocket },
            ].map((s, i) => (
              <div key={s.id} className="flex items-center">
                <button
                  onClick={() => step > s.id && setStep(s.id)}
                  className={`flex items-center gap-3 px-6 py-3 rounded-[20px] transition-all duration-300 ${step === s.id
                      ? "bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 shadow-[0_0_30px_rgba(34,211,238,0.15),inset_0_0_10px_rgba(34,211,238,0.1)] border border-cyan-500/30 font-black"
                      : step > s.id
                        ? "text-blue-400 hover:bg-blue-500/10 hover:shadow-[0_0_20px_rgba(59,130,246,0.1)]"
                        : "text-zinc-500 cursor-not-allowed"
                    }`}
                >
                  <s.icon size={18} className={step === s.id ? "animate-pulse shadow-[0_0_10px_currentColor] rounded-full" : ""} />
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              <div className="flex flex-col md:flex-row gap-6 items-center gf-panel-strong p-4 rounded-[32px] border border-cyan-500/20 backdrop-blur-2xl bg-[var(--gf-shell-bg)]/80 shadow-[0_20px_60px_-10px_rgba(0,0,0,0.6)]">
                <div className="relative flex-1 w-full group">
                  <div className="absolute inset-0 bg-cyan-500/10 blur-2xl opacity-0 group-focus-within:opacity-100 transition-opacity pointer-events-none" />
                  <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-cyan-500/50 group-focus-within:text-cyan-400 transition-colors pointer-events-none" size={20} />
                  <input
                    className="w-full rounded-[24px] pl-16 pr-6 h-14 text-base border border-transparent focus:border-cyan-500/30 bg-white/[0.02] focus:bg-[var(--gf-shell-bg)]/60 transition-all font-bold text-white placeholder:text-zinc-600 outline-none"
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          className={`gf-panel-strong group relative overflow-hidden rounded-[36px] border-2 transition-all duration-500 text-left w-full h-full flex flex-col ${active
                              ? "border-cyan-400/50 bg-[var(--gf-shell-bg)]/90 shadow-[0_20px_60px_-10px_rgba(34,211,238,0.4),0_0_20px_rgba(34,211,238,0.15)] scale-[1.02]"
                              : "border-white/[0.03] bg-[var(--gf-shell-bg)]/50 hover:bg-[var(--gf-shell-bg)]/80 hover:border-cyan-500/20 hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)]"
                            }`}
                        >
                          <div className="aspect-[16/11] relative overflow-hidden m-3 rounded-[32px] bg-black/40">
                            ...
                            <div className="absolute top-4 left-4">
                              <span className="rounded-xl border border-white/10 bg-black/60 backdrop-blur-xl px-4 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-blue-300 shadow-2xl">
                                {t.category}
                              </span>
                            </div>

                            <div className="absolute top-4 right-4">
                              <span className="rounded-xl border border-white/10 bg-black/70 backdrop-blur-xl px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] text-blue-200 shadow-2xl">
                                {Number(t.price || 0) > 0 ? `$${Number(t.price || 0).toFixed(2)}` : "Free"}
                              </span>
                            </div>
                            ...
                            {active && (
                              <motion.div
                                ...
                                className="absolute inset-0 border-[3px] border-cyan-400 rounded-[32px] pointer-events-none shadow-[inset_0_0_40px_rgba(34,211,238,0.3)]"
                              >
                                <div className="absolute top-4 right-4 h-10 w-10 rounded-[14px] bg-blue-500 flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.8)] border border-blue-400/20">
                          ...
                          {active && (
                            <motion.div
                              ...
                              className="absolute inset-0 bg-cyan-500/30 blur-[80px] -z-10 pointer-events-none"
                            />
                          )}
                        </button>
                      </Tilt>
          ...
          {step === 2 && (
            <motion.div
              ...
              className="grid grid-cols-1 lg:grid-cols-12 lg:items-start gap-10"
            >
              <div className="lg:col-span-8 lg:min-w-0 space-y-8">
                {/* Visual DNA Config */}
                <motion.div
                  ...
                  <div className="grid grid-cols-1 gap-12 relative z-10">
                    <div className="space-y-6">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Selected: {COLOR_PRESETS.find(p => p.primary === config.primaryColor)?.label || "Custom"}</span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                        {COLOR_PRESETS.map((p, idx) => (
                          <motion.button
                            ...
                            className={`flex flex-col gap-3 p-2 rounded-[24px] border-2 transition-all ${config.primaryColor === p.primary && config.secondaryColor === p.secondary
                                ? "border-blue-500 bg-blue-500/10 shadow-[0_0_40px_rgba(99,102,241,0.2)]"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-4 group/input">
                        <label className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] group-focus-within/input:text-blue-400 transition-colors">Project Identity</label>
                        <div className="relative">
                          <input
                            className="gf-input w-full rounded-[24px] px-8 py-5 bg-black/40 border-2 border-white/5 text-xl font-black italic tracking-tight text-white placeholder:text-zinc-800 focus:border-blue-500/50 transition-all shadow-inner"
                            value={config.name}
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-4">
                          {[
                            { id: "webgl", icon: Globe, label: "WEB RUNTIME", color: "text-blue-400" },
                            { id: "android", icon: MonitorSmartphone, label: "APK EXPORT", color: "text-blue-400" }
                          ].map((target) => (
                            <motion.button
                              key={target.id}
                              ...
                              className={`flex-1 flex flex-col items-center justify-center gap-3 py-5 rounded-[24px] border-2 transition-all ${config.buildTarget === target.id
                ...
                >
                    ...
                    <div className="space-y-12">
                      <div className="space-y-6">
                        <div className="flex justify-between items-end">
                          ...
                          <span className="text-blue-400 font-black text-2xl italic tracking-tighter">{config.speed.toFixed(1)}<span className="text-xs ml-1">m/s</span></span>
                        </div>
                        <div className="relative py-4">
                          <input
                            type="range" min="1" max="20" step="0.5"
                            className="w-full h-2 appearance-none bg-white/5 rounded-full accent-blue-500 cursor-pointer"
                            value={config.speed}
                    ...
                    <div className="space-y-12">
                      <div className="space-y-6">
                        <div className="flex justify-between items-end">
                          ...
                          <span className="text-blue-300 font-black text-2xl italic tracking-tighter">{config.jumpForce.toFixed(1)}<span className="text-xs ml-1">N</span></span>
                        </div>
                        <div className="relative py-4">
                          <input
                      ...
                      <div className="space-y-6">
                        <div className="flex justify-between items-end">
                          ...
                          <span className="text-blue-400 font-black text-2xl italic tracking-tighter">{config.timeScale.toFixed(1)}<span className="text-xs ml-1">x</span></span>
                        </div>
                        <div className="relative py-4">
                          <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 border-t-2 border-white/5 pt-12">
                    <motion.button
                      ...
                      className={`group p-8 rounded-[32px] border-2 flex items-center justify-between transition-all duration-500 ${config.useAdvancedPhysics ? "border-blue-500/50 bg-blue-500/[0.05] shadow-2xl" : "border-white/5 bg-white/[0.02]"
                        }`}
                    >
                      <div className="flex items-center gap-5">
                        <div className={`h-14 w-14 rounded-2xl flex items-center justify-center transition-all ${config.useAdvancedPhysics ? "bg-blue-500 text-white shadow-[0_0_20px_#6366f1]" : "bg-white/5 text-zinc-700"}`}>
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-[10px] text-blue-400/60 font-black uppercase tracking-widest mt-1">High Fidelity Collisions</div>
                        </div>
                      </div>
                      <div className={`h-8 w-8 rounded-full border-2 flex items-center justify-center transition-all ${config.useAdvancedPhysics ? "border-blue-500 bg-blue-500" : "border-zinc-800"
                        }`}>
                    ...
                    <motion.button
                      ...
                      className={`group p-8 rounded-[32px] border-2 flex items-center justify-between transition-all duration-500 ${config.enableMultiplayer ? "border-cyan-500/50 bg-cyan-500/[0.05] shadow-2xl" : "border-white/5 bg-white/[0.02]"
                        }`}
                    >
                      <div className="flex items-center gap-5">
                      ...
                      <div className={`h-8 w-8 rounded-full border-2 flex items-center justify-center transition-all ${config.enableMultiplayer ? "border-cyan-500 bg-cyan-500" : "border-zinc-800"
                        }`}>
              ...
              <div className="lg:col-span-4">
                <motion.div
                  ...
                  className="gf-panel-strong gf-stroke-gradient rounded-[48px] p-10 shadow-[0_0_100px_rgba(0,0,0,0.5)] border-2 border-white/5"
                >
                  ...
                  <div className="flex items-center justify-between mb-10 relative z-10">
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 2, repeat: Infinity }}
                      className="h-2.5 w-2.5 rounded-full bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,1)]"
                    />
                  </div>
                  ...
                  <div className="space-y-8 relative z-10">
                    <div className="aspect-[16/11] rounded-[36px] overflow-hidden border-2 border-white/10 relative group bg-black shadow-2xl">
                      ...
                      <div className="absolute bottom-6 left-6 right-6">
                        <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-2 drop-shadow-lg">Active Core</div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2 col-span-2">
                        <div className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.4em] leading-none">Template Price</div>
                        <div className="text-sm font-black text-white uppercase tracking-wide">
                          {selectedTemplatePrice > 0 ? `$${selectedTemplatePrice.toFixed(2)}` : "Free"}
                        </div>
                        {Number(selectedTemplate?.isDiscounted) ? (
                          <div className="text-[10px] text-blue-300">
                            Discount applied from ${Number(selectedTemplate?.originalPrice || selectedTemplatePrice).toFixed(2)}
                          </div>
                        ) : null}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="h-3 w-full rounded-full bg-white/5 overflow-hidden p-0.5 border border-white/5">
                          <motion.div
                            ...
                            className="h-full bg-gradient-to-r from-blue-700 to-sky-500 rounded-full"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-8">
                      ...
                      >
                        <span className="relative z-10 flex items-center gap-3">
                          {selectedTemplatePrice > 0 ? "Pay & Launch Neural Build" : "Launch Neural Build"} <Rocket size={20} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                        </span>
                        <motion.div
                          animate={{ x: ["-100%", "200%"] }}
