...
const INITIAL_ACTIVITIES: ActivityItem[] = [
  { id: "1", user: "CyberNeon", action: "deployed", target: "Space Runner v2", time: "Just now", icon: Rocket, color: "text-blue-400" },
  { id: "2", user: "MarioDev", action: "published", target: "Retro World", time: "2m ago", icon: Layers, color: "text-blue-400" },
...
export default function GlobalActivityFeed() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const colors = ["text-blue-400", "text-blue-400", "text-cyan-400", "text-emerald-400", "text-rose-400"];
      ...
      <AnimatePresence initial={false}>
        {activities.map((a) => (
          <motion.div
            ...
            className="gf-panel flex items-center gap-4 p-4 rounded-2xl border border-black/5 dark:border-white/5 bg-white/[0.03] dark:bg-white/[0.01] hover:bg-white/[0.06] dark:hover:bg-white/[0.03] transition-all overflow-hidden shadow-sm"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-bold text-zinc-900 dark:text-white truncate">
                <span className="font-black text-blue-600 dark:text-blue-400">{a.user}</span> {a.action}{" "}
                <span className="text-zinc-500 dark:text-zinc-400 font-black">{a.target}</span>
              </div>
